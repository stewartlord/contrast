...
swarm.jobs = {
    ...
    init: function(attachListeners) {
        ...
        var table  = $('table.jobs'),
            search = $('.jobs-toolbar input[name=query]');
        table.data('query', search.val());
        swarm.jobs.load();

        // wire-up search filter
        var runSearch = function() {
            // if search hasn't changed, return
            if (table.data('query') === search.val()) {
                return;
            }
        ...
        $('table.jobs.sortable thead tr').sortable({
            ...
            onDrop: function(item, container, _super) {
                ...
                var fields = item.parent().find('th').map(function(){
            ...
            onDragStart: function(item, container, _super) {
                ...
                table.find('tbody').addClass('sorting');
            },
        ...
        table.on('loaded', function(){
            swarm.jobs.initFieldSelector(
                table.find('thead').data('spec'),
                true,
                swarm.jobs.getSelectedFields,
                function(fields){
                    swarm.localStorage.set('jobs.fields', fields);
                    swarm.jobs.refresh(table);
                }
    ...
    initFieldSelector: function(spec, sortable, getSelectedFields, onSelect){
        var buttonSelector = '.btn-select-fields',
            button         = $(buttonSelector);

        button.prop('disabled', false);

        var toggleSelector = function (forceClose) {
            ...
            if (button.is('.active')) {
                swarm.jobs.openFieldSelector(button, spec, sortable, getSelectedFields, onSelect);
            }
        };
        ...
        $(document).on(
            'click.job-fields',
            buttonSelector + ', .popover-job-fields',
            function (e) { e.stopPropagation(); }
        );
    },

    getSelectedFields: function(spec) {
        ...
        return [
            spec.__id,
            spec.__status,
            spec.__createdBy    || spec.__modifiedBy,
            spec.__description,
            spec.__modifiedDate || spec.__createdDate
        ];
    },

    openFieldSelector: function(element, spec, sortable, getSelectedFields, onSelect) {
        element = $(element);

        var table    = $('table.jobs'),
            selected = getSelectedFields(spec);

        // if fields are sortable, always put selected fields first
        var fields = sortable ? selected.slice() : [];

        // add other available fields
        $.each(spec, function(field){
            if (field.indexOf('__') !== 0 && $.inArray(field, fields) === -1) {
                fields.push(field);
            }
        });
        ...
        $.each(fields, function(index, field){
            fieldsList += $.templates(
                ...
                +   '<input type="checkbox" data-value="{{>field}}" {{if checked}}checked="checked"{{/if}}>'
                +   '{{>label}}'
                +  '</label>'
                + '</li>'
            ).render({
                ...
                checked: $.inArray(field, selected) !== -1,
                label:   swarm.jobs.getFriendlyLabel(field)
            });
        });

        // prepare content for the popover
        var content = '<ul class="unstyled pad1 padw0 field-list">' + fieldsList + '</ul>';
        ...
        var popover   = $(element.data('popover').tip()),
            fieldList = popover.find('.field-list');

        // prepare handler to apply changes in job columns
        var handleApply = function() {
            var fields = [];
            fieldList.find(':checked').each(function(){
        ...
        // if not sortable, then all done
        if (!sortable) {
            return;
        }

        // make fields sortable
        fieldList.addClass('sortable');
        fieldList.sortable({
            nested: false,
            onDrop: function(item, container, _super) {
                _super(item);
            ...
            onDrag: function(item, position) {
                // lock the dragged item inside the field-list container
                var itemHalfHeight = parseInt(item.height() * 0.5, 10),
                    lowerBound     = itemHalfHeight * 2,
                    upperBound     = fieldList.height() + itemHalfHeight,
    ...
    refresh: function(table, jobs) {
        var thead  = $(table).find('thead'),
            ...
            fields = swarm.jobs.getSelectedFields(spec);
